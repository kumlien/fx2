...
public class RobotDefintionCRUDTest {
	...
	TwoIndicatorCondition buyCondition = new TwoIndicatorCondition("Buy when ask is >= 150", new CurrentAskIndicator(), new SimpleValueIndicator(150.0), Comparator.GREATER_OR_EQUAL_THAN, 1, Side.BUY, MarketUpdateType.ONE_MINUTE_CANDLE);
	...
	TwoIndicatorCondition stopLoss = new TwoIndicatorCondition("Sell when ask is <= 140", new CurrentAskIndicator(), new SimpleValueIndicator(140.0), Comparator.LESS_OR_EQUAL_THAN, 1, Side.BUY);
	TwoIndicatorCondition takeProfit = new TwoIndicatorCondition("Sell when bid is >= 160", new CurrentBidIndicator(), new SimpleValueIndicator(160.0), Comparator.GREATER_OR_EQUAL_THAN, 2, Side.BUY);
	...
	TwoIndicatorCondition buyCondition = new TwoIndicatorCondition("Buy when ask is >= 150", new CurrentAskIndicator(), new SimpleValueIndicator(150.0), Comparator.GREATER_OR_EQUAL_THAN, 1, Side.BUY);
	rd.addBuyCondition(buyCondition);
	rd = robotRepo.save(rd);
	Assert.assertNotNull(rd.getId());
	LOG.info("RobotDefinition saved to db with id {}", rd.getId());
