...
import hoggaster.oanda.responses.OandaCandle;
...
public class Robot implements Consumer<Event<?>> {
	...
	@Autowired
	public Robot(Depot depot, RobotDefinition definition, MovingAverageService maService, @Qualifier("priceEventBus") EventBus priceEventBus, Broker broker) {
	...
	public void start() {
		...
		eventBusRegistrations.put("priceRegistration", priceEventBus.on(Selectors.R("prices." + instrument.name() + "*"), this));
	...
	public void onNewCandle(OandaCandle candle) {
	...
	public void accept(Event<?> t) {
		...
		} else if (t.getData() instanceof OandaCandle) {
			onNewCandle((OandaCandle) t.getData());
		}
	}
